...
(function () {
  ...
  Dynamsoft.OnWebTwainNotFoundOnWindowsCallback = function (
    ...
    console.warn('OnWebTwainNotFoundOnWindowsCallback');
    var _this = Dynamsoft,
      objUrl = { default: InstallerUrl };
    _this._show_install_dialog(
      ProductName,
      objUrl,
      bHTML5,
      Dynamsoft.DWT.EnumDWT_PlatformType.enumWindow,
      bIE,
      bSafari,
      bSSL,
      strIEVersion,
  ...
  Dynamsoft.OnWebTwainNotFoundOnLinuxCallback = function (
    ...
    console.warn('OnWebTwainNotFoundOnLinuxCallback');
    var _this = Dynamsoft,
      objUrl = { default: strDebUrl, deb: strDebUrl, rpm: strRpmUrl };
  ...
  Dynamsoft.OnWebTwainNotFoundOnMacCallback = function (
    ...
    console.warn('OnWebTwainNotFoundOnMacCallback');
    var _this = Dynamsoft,
      objUrl = { default: InstallerUrl };
    _this._show_install_dialog(
      ProductName,
      objUrl,
      bHTML5,
      Dynamsoft.DWT.EnumDWT_PlatformType.enumMac,
      bIE,
      bSafari,
      bSSL,
      strIEVersion,
  ...
  Dynamsoft.dwt_change_install_url = function (url) {
    console.warn('dwt_change_install_url');
    var install = document.getElementById('dwt-btn-install');
    if (install) install.href = url;
  };

  Dynamsoft.DWT_Reconnect = function () {
    console.warn('DWT_Reconnect');
    var _this = Dynamsoft;
    if ((new Date() - reconnectTime) / 1000 > 30) {
      ...
      if (el) {
        el.innerHTML =
          'Failed to connect to the service, have you run the setup?<br />If not, please run the setup and <a href="javascript:void(0)" onclick="Dynamsoft.DCP_DWT_onclickInstallButton()">click here to connect again</a>.';
      }
      return;
    }
    if (_this.DWT) {
      _this.DWT.CheckConnectToTheService(
        function () {
          Dynamsoft.DWT.ConnectToTheService();
        },
        function () {
          if (
            Dynamsoft._isInsecurePrivateNetwork() &&
            Dynamsoft._isChrome94plus()
          ) {
            Dynamsoft._checkDWTByWebSocket(
              function () {
  ...
  Dynamsoft.DCP_DWT_onclickInstallButton = function () {
    console.warn('DCP_DWT_onclickInstallButton');
  ...
  Dynamsoft._isChrome94plus = function () {
    ...
    if (_nChrome > -1) {
      ...
      _tmp = ver.indexOf(' ');
      if (_tmp > -1) ver = parseInt(ver.slice(0, _tmp));
    } else {
  ...
  Dynamsoft._checkDWTByWebSocket = function (
    ...
    var urls = Dynamsoft.Lib.detect.urls,
      ws;
    ...
    ws.onopen = function () {
      if (ws.readyState == 1) {
        ws.onopen = null;
        var strPostStringData = [
          '{"id":"',
          new Date().getTime(),
          '","method":"VersionInfo","version":"',
          Dynamsoft.Lib.product.getDWTPrefix(),
          Dynamsoft.Lib.product.getServerVersionInfo(),
          '"}',
        ].join('');
    ...
    ws.onmessage = function (msg) {
      try {
        ws.close();
        if (msg.data.indexOf('Version not found') >= 0) callbackVersionFailed();
  ...
  Dynamsoft._show_install_dialog = function (
    ...
    console.warn('_show_install_dialog');
    var _this = Dynamsoft,
      ObjString,
      title,
      browserActionNeeded,
      ...
      bUnix =
        iPlatform == EnumPlatform.enumLinux ||
        iPlatform == EnumPlatform.enumEmbed ||
        iPlatform == EnumPlatform.enumChromeOS ||
        iPlatform == EnumPlatform.enumHarmonyOS;

    if (Dynamsoft.DWT.bNpm && Dynamsoft.navInfoSync.bFileSystem) {
      return Dynamsoft._show_online_download_dialog(
        objInstallerUrl,
        bHTML5,
        iPlatform
      );
    ...
    ObjString = ['<div class="dynamsoft-dwt-dlg-title">', title, '</div>'];
    ...
    if (_this.DWT) {
      if (
        ...
        Dynamsoft._checkDWTByWebSocket(
          ...
          function () {
            Dynamsoft._show_install_dialog(
              ProductName,
              objInstallerUrl,
              bHTML5,
              iPlatform,
      ...
      ObjString.push(
        '<div class="dynamsoft-dwt-installdlg-splitline" style="left: 125px"></div>'
      );
      ObjString.push(
        '<div class="dynamsoft-dwt-installdlg-splitline" style="left: 283px"></div>'
      );
      ObjString.push(
        '<img style="margin: 0px 134px 0px 0px" src=' +
          imagesInBase64.icn_download +
          ' alt="download">'
      );
      ObjString.push(
        '<img style="margin: 2px 132px 2px 0px" src=' +
          imagesInBase64.icn_install +
          ' alt="install">'
      );
      ObjString.push('<img src=' + imagesInBase64.icn_scan + ' alt="scan">');
      ObjString.push(
        '<div><span class="dynamsoft-dwt-installdlg-text" style="right: 125px">Download</span>'
      );
      ObjString.push(
        '<span class="dynamsoft-dwt-installdlg-text" style="right: 18px">Install</span>'
      );
      ObjString.push(
        '<span class="dynamsoft-dwt-installdlg-text" style="left: 105px">Scan</span>'
      );
      ObjString.push('</div>');
      ObjString.push('</div>');
      ...
      if (bHTML5 && bUnix) {
        ...
        if (arm64 || mips64 || chromeOS || harmonyOS) {
        } else {
          ObjString.push(
            '<div><input id="dwt-install-url-deb" name="dwt-install-url" type="radio" onclick="Dynamsoft.dwt_change_install_url(\'' +
              objInstallerUrl.deb +
              '\')" checked="checked" /><label for="dwt-install-url-deb">64 bit .deb (For Ubuntu/Debian)</label></div>'
          );
          ObjString.push(
            '<div><input id="dwt-install-url-rpm" name="dwt-install-url" type="radio" onclick="Dynamsoft.dwt_change_install_url(\'' +
              objInstallerUrl.rpm +
              '\')" /><label for="dwt-install-url-rpm">64 bit .rpm (For Fedora)</label></div>'
          );
        }
      ...
      ObjString.push(
        ' onclick="Dynamsoft.DCP_DWT_onclickInstallButton()"><div class="dynamsoft-dwt-dlg-button">Download</div></a></div>'
      );
      if (bHTML5) {
        if (bIE) {
          ObjString.push(
            '<div class="dynamsoft-dwt-dlg-tail" style="text-align:left; padding-left: 80px">'
          );
          ObjString.push(
            'If you still see the dialog after installing the scan service, please<br />'
          );
          ObjString.push(
            '1. Add the website to the zone of trusted sites.<br />'
          );
          ObjString.push(
          ...
          if (bUnix) {
            ...
            ObjString.push(
              '<div class="dynamsoft-dwt-dlg-red">After the installation, please <strong>' +
                browserActionNeeded +
                '</strong>  your browser.</div>'
            );
            ObjString.push('</div>');
          }
        }
        ...
        if (bIE) {
          ...
          ObjString.push(
            '2. Allow "DynamicWebTWAIN" add-on to run by right clicking on the Information Bar in the browser.'
          );
        } else {
          ObjString.push(
            '<div class="dynamsoft-dwt-dlg-red">After installation, please <strong>REFRESH</strong> your browser.</div>'
          );
        }
        ObjString.push('</div>');
      }
  ...
  Dynamsoft.OnWebTwainNeedUpgradeCallback = function (
    ProductName,
    objInstallerUrl,
    bHTML5,
    iPlatform,
  ...
  Dynamsoft.OnRemoteWebTwainNotFoundCallback = function (
    ProductName,
    ip,
    port,
    bSSL,
    ...
    console.warn('OnRemoteWebTwainNotFoundCallback');
    var _this = Dynamsoft;
    var ObjString;
  ...
  Dynamsoft.OnRemoteWebTwainNeedUpgradeCallback = function (
    ProductName,
    ip,
    port,
    bSSL
  ) {
    console.warn('OnRemoteWebTwainNeedUpgradeCallback');
    var _this = Dynamsoft;
    var ObjString = [
  ...
  Dynamsoft.OnWebTWAINDllDownloadFailure = function (
    ProductName,
    errorCode,
    errorString
  ) {
    console.warn('OnWebTWAINDllDownloadFailure');
    var _this = Dynamsoft;
    if (_this.DWT) {
  ...
  Dynamsoft.OnGetServiceUpdateStatus = function (
    bError,
    statusCode,
    statusString
  ) {
    console.warn('OnGetServiceUpdateStatus');
    if (statusString != 'Update skipped') console.log(statusString);
  };

  Dynamsoft.OnWebTWAINModuleDownloadManually = function (
    objInstallerUrl,
    iPlatform,
    bIE,
    bSafari,
    ...
    console.warn('OnWebTWAINModuleDownloadManually');
    var _this = Dynamsoft;
    return _this._show_install_dialog(
      '',
      objInstallerUrl,
      true,
      iPlatform,
  ...
  Dynamsoft.OnLTSLicenseError = function (message) {
    console.warn('OnLTSLicenseError');
    var addMessage = '',
      ObjString,
      lexp = Dynamsoft.DWT.licenseException,
      code = 0;
    if (lexp) code = lexp.code;

    if (
      code == -2440 || // NetworkError
      code == -2441 || // Timedout
      code == -2443 || // CorsError
      ...
      var purchaseUrl =
        'https://www.dynamsoft.com/customer/license/trialLicense?product=dwt&deploymenttype=js';
      addMessage =
        '<div>You can register for a free 30-day trial <a href="' +
        purchaseUrl +
        '" target="_blank" class="dynamsoft-major-color">here</a>. Make sure to select the product Dynamic Web TWAIN.</div>';
    }

    ObjString = [message, addMessage];
  ...
  Dynamsoft.OnLTSConnectionWarning = function () {
    console.warn('OnLTSConnectionWarning');
    var ObjString = [
      ...
      Dynamsoft.DWT.isPublicLicense()
        ? '<a class="dynamsoft-major-color" href="https://www.dynamsoft.com/company/contact/">Contact Dynamsoft</a> '
  ...
  Dynamsoft.OnLTSPublicLicenseMessage = function (message) {
    console.warn('OnLTSPublicLicenseMessage');
  ...
  Dynamsoft.OnLicenseExpiredMessage = function (strExpiredDate, remain) {
    console.log('OnLicenseExpiredMessage');
    var ObjString,
      a_contact_us =
        '<a target="_blank" href="https://www.dynamsoft.com/company/contact/">contact us</a>',
      a_online_store =
        '<a target="_blank" href="https://www.dynamsoft.com/store/dynamic-web-twain/#DynamicWebTWAIN">online store</a>',
      a_new_key_href =
        'https://www.dynamsoft.com/customer/license/trialLicense?product=dwt&utm_source=in-product';
